...
#--- for RL
edovo_user_and_facility <- edovo_user_and_facility %>% 
  mutate(facility_name = name) %>% select(-name)
for_RL <- edovo_user_data %>% select( -hashed_password, 
                                     ...
                                     -zendesk_id,
                                     -release_date) %>%
  mutate(original_edovo_id = id,
         original_inmate_id = inmate_id) %>%
  select(-id, -inmate_id) %>%
...
for(i in 1:10){ ## Just to diagnose, let's try this just in 3 blocks
...
all.files <- map_chr(1:9,  function(i)  paste0("./data_", "C", str_pad(i, 3, pad = "0"), "_Dedup.RData"))
...
all_blocks <- do.call("bind_rows", mylist)

all_blocks <- all_blocks %>%
  mutate(inmate_id= as.numeric(inmate_id))
